 ...
 */
const createHanuiModule = (require) => {
    ...
    const PRODUCT = {
      bar: 'dshProductBar',
      mark: 'dshProductMark',
      identity: 'dshProductIdentity',
      section: 'dshProductSection',
      state: 'dshProductState',
    }
    const MOBILE_SAFE_TOP = 'max(44px, env(safe-area-inset-top, 0px))'
    const MOBILE_DRAWER_WIDTH = 'min(86vw, 340px)'
...
@media ${MOBILE_MQ} {
  ...
  html.${HTML_CLASS} .${CLS.sidebar} {
    ...
    right: auto !important;
    bottom: 0 !important;
    width: ${MOBILE_DRAWER_WIDTH} !important;
  ...
  /* Settings panel: desktop is an 800px two-column sheet (188px nav + content).
     On phones it becomes a compact sheet below the status bar. The original
     desktop option rows are intentionally tightened here instead of scaling
     the entire panel, which preserves readable labels and touch targets. */
  html.${HTML_CLASS} {
    --dsh-mobile-safe-top: ${MOBILE_SAFE_TOP};
  }
  ...
  html.${HTML_CLASS} .${CLS.sidebar}:has(.${SETTINGS.overlay}) {
    /* The settings slot is mounted inside the legacy sidebar. Let the
       full-width mobile sheet escape the drawer's scroll clipping. */
    overflow: visible !important;
  }
  html.${HTML_CLASS} .${SETTINGS.panel} {
    width: 100% !important;
    max-width: 100% !important;
    height: calc(100dvh - var(--dsh-mobile-safe-top)) !important;
    max-height: calc(100dvh - var(--dsh-mobile-safe-top)) !important;
    margin: var(--dsh-mobile-safe-top) 0 0 !important;
  ...
  html.${HTML_CLASS} .${SETTINGS.panel} .${SETTINGS.nav} .${SETTINGS.navLabel} {
  ...
  html.${HTML_CLASS} .${SETTINGS.panel} .${SETTINGS.header} {
    flex: none !important;
  ...
  html.${HTML_CLASS} .${SETTINGS.panel} .${SETTINGS.close} {
  ...
  html.${HTML_CLASS} .${SETTINGS.panel} .${SETTINGS.options} [class$="_row"] {
    width: 100% !important;
  ...
  html.${HTML_CLASS} .${SETTINGS.panel} .${SETTINGS.options} [class$="_rowText"] {
    min-width: 0 !important;
    gap: 4px !important;
  }
  html.${HTML_CLASS} .${SETTINGS.panel} .${SETTINGS.options} [class$="_title"] {
    font-size: 15px !important;
    line-height: 20px !important;
  }
  html.${HTML_CLASS} .${SETTINGS.panel} .${SETTINGS.options} [class$="_desc"] {
    font-size: 12px !important;
    line-height: 17px !important;
  }
  html.${HTML_CLASS} .${SETTINGS.panel} .${SETTINGS.options} [class$="_selector"] {
    flex: 0 1 auto !important;
    min-width: 0 !important;
    min-height: 36px !important;
    height: 36px !important;
  ...
  html.${HTML_CLASS} .${SETTINGS.panel} .${SETTINGS.options} [class$="_group"] {
    padding-top: 12px !important;
    padding-bottom: 12px !important;
  }
  html.${HTML_CLASS} .${SETTINGS.panel} .${SETTINGS.options} [class$="_cubeRow"] {
    display: grid !important;
    grid-template-columns: repeat(3, minmax(0, 1fr)) !important;
    gap: 8px !important;
  }
  html.${HTML_CLASS} .${SETTINGS.panel} .${SETTINGS.options} [class$="_themeCube"] {
    min-width: 0 !important;
  ...
  /* A narrower drawer leaves a deliberate thumb-reachable content column and
     keeps the dimmed workbench visible as context. */
  html.${HTML_CLASS} .${SIDEBAR_ROOT} .hHd-Xa_logoRow {
    min-height: 56px !important;
    height: 56px !important;
    padding: 10px 14px !important;
  }
  html.${HTML_CLASS} .${SIDEBAR_ROOT} .hHd-Xa_newSession {
    min-height: 44px !important;
    height: 44px !important;
  ...
  html.${HTML_CLASS} .${SIDEBAR_ROOT} .hHd-Xa_footArea,
  html.${HTML_CLASS} .${SIDEBAR_ROOT} .hHd-Xa_settingsArea {
    min-height: 76px !important;
    height: 76px !important;
  }

  /* Product surface — keep the mobile workbench quiet and ink-led. */
  html.${HTML_CLASS} .${CLS.center} {
    background: var(--dsw-alias-bg-base, #fafafa) !important;
    color: var(--dsw-alias-label-primary, #1a1a1a) !important;
  }
  html.${HTML_CLASS} .${HDR.header} {
    background: var(--dsw-alias-bg-base, #fafafa) !important;
    border-bottom: 1px solid var(--dsw-alias-border-l2, #e0e0e0) !important;
  }
  html.${HTML_CLASS} .${HDR.tabs} [aria-selected="true"],
  html.${HTML_CLASS} .${HDR.tabs} [data-state="active"],
  html.${HTML_CLASS} .${HDR.tabs} [data-active="true"] {
    color: var(--dsw-alias-label-primary, #1a1a1a) !important;
    border-color: var(--dsw-alias-label-primary, #1a1a1a) !important;
  }
  html.${HTML_CLASS} .${HDR.tabs} [aria-selected="true"]::after,
  html.${HTML_CLASS} .${HDR.tabs} [data-state="active"]::after,
  html.${HTML_CLASS} .${HDR.tabs} [data-active="true"]::after {
    background: var(--dsw-alias-label-primary, #1a1a1a) !important;
  }
  html.${HTML_CLASS} .${HDR.headerActions},
  html.${HTML_CLASS} .${HDR.headerUtilities} {
  ...
  /* The drawer is our product shell now; keep the existing new-session
     action, but remove the inherited DeepSeek wordmark from its visual chrome. */
  html.${HTML_CLASS} .${SIDEBAR_ROOT} .hHd-Xa_logoRow {
    align-items: center !important;
    gap: 8px !important;
    padding: 12px 16px !important;
  }
  html.${HTML_CLASS} .${SIDEBAR_ROOT} .hHd-Xa_brand {
    display: inline-flex !important;
    align-items: center !important;
    gap: 8px !important;
    width: auto !important;
  ...
  html.${HTML_CLASS} .${SIDEBAR_ROOT} .hHd-Xa_brand svg {
    display: none !important;
  }
  html.${HTML_CLASS} .${SIDEBAR_ROOT} .hHd-Xa_brand::before {
    content: 'DH';
    display: inline-grid;
    place-items: center;
    width: 24px;
    ...
    color: var(--dsw-alias-bg-base, #fafafa);
    font-size: 10px;
    letter-spacing: 0.04em;
  }
  html.${HTML_CLASS} .${SIDEBAR_ROOT} .hHd-Xa_brand::after {
    content: 'DeepHarness';
  }
...
.dshMobMenu:active {
  background: var(--dsw-alias-button-floating-hover, rgba(0,0,0,.06));
}
.dshMobMenuIcon {
  display: flex;
...
.dshProductBar {
  ...
  border-bottom: 1px solid var(--dsw-alias-border-l2, #e0e0e0);
  color: var(--dsw-alias-label-primary, #0f1115);
  pointer-events: none;
}
.dshProductIdentity {
...
@media ${MOBILE_MQ} {
  ...
  .dshProductBar[data-visible="true"] {
    display: flex;
  }
  html.${HTML_CLASS} .${CLS.center} {
    box-sizing: border-box !important;
    /* center already starts below the device safe area; only reserve the
       product bar here, otherwise the safe area is counted twice. */
          ...
          jsxs('div', {
            ...
            children: [
              jsxs('div', {
                ...
                children: [
                  jsx('span', { className: PRODUCT.mark, children: 'DH' }),
                  jsx('span', { children: 'DeepHarness' }),
                  jsx('span', { className: PRODUCT.section, children: '工作台' }),
                ],
              }),
              jsx('span', { className: PRODUCT.state, children: '已连接' }),
            ],
          }),
          jsx('button', {
            type: 'button',
