...
`)
	...
	if fp, err := cryptoutil.CertSHA256FingerprintFile(certPath); err == nil {
		...
		fmt.Printf("Plugin: check “允许自签 TLS”, then paste this fingerprint.\n")
	}
	if listenAll {
...
func initLayout(dir string, force, listenAll bool, extraHosts []string) (password, configPath string, err error) {
	...
	publicHost := firstPublicHost(extraHosts)
	publicHostLine := ""
	if publicHost != "" {
		publicHostLine = fmt.Sprintf("public_host = %s\n", tomlQuote(publicHost))
	}
	publicHostLine += "# HTTPS Control URL for hosted tenants (reverse proxy to 127.0.0.1:8080).\n# public_control_url = \"https://control.example.com\"\n"
