...
Relay 的数据面是实时字节管道：它不解析内层 HTTP，也不持久化聊天正文、Prompt、文件、工作区、审批内容或响应正文。在线 Registry、连接流、挑战和限流桶只存在于 Relay 进程内存中，Relay 重启后由 Agent 重新注册。

Control 控制面为了鉴权、撤销和运维，会在 SQLite 中保存最小控制元数据：Host/Route/设备标识、公钥、凭证哈希及有效期、撤销状态、在线心跳、聚合流量/连接统计和控制台审计事件。Control 不保存会话正文；Relay 也不读取 Control 数据库或签发主密钥。

因此，对外准确的表述是：**Relay 不存储业务内容，仅保存必要的控制元数据。** 它不是“完全不存储任何数据”的匿名转发服务。
