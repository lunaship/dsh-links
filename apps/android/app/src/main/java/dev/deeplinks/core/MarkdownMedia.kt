...
object MarkdownMedia {
    ...
    val publicOnlyDns = object : okhttp3.Dns {
        override fun lookup(hostname: String): List<InetAddress> {
            val addrs = okhttp3.Dns.SYSTEM.lookup(hostname)
            if (addrs.isEmpty() || addrs.any { isBlockedInetAddress(it) }) {
                throw java.net.UnknownHostException("blocked markdown image destination")
            }
            return addrs
        }
    }
    ...
    internal fun isBlockedHostname(host: String): Boolean {
        ...
        // Decimal / hex / dotted-numeric aliases (127.1, 2130706433, 0x7f000001, 0177.0.0.1)
        // must not skip the IPv4 private-range check by looking like a hostname.
        if (host.all { it.isDigit() }) return true
        if (host.startsWith("0x") || host.startsWith("0X")) return true
        val dotted = host.split('.')
        if (dotted.isNotEmpty() && dotted.all { part -> part.isNotEmpty() && part.all { it.isDigit() } }) {
            if (dotted.size != 4) return true
            if (dotted.any { it.length > 1 && it.startsWith("0") }) return true
            parseIpv4(host)?.let { return isBlockedIpv4(it) }
            return true
        }
        return false
