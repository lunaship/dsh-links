...
import dev.deeplinks.core.verifyPatchApplied
import dev.deeplinks.core.withNamespace
...
class AppSettingsTest {
    ...
    @Test
    fun `服务端 default 为 JSON null 时回落 standard 而不是字面量 null`() {
        val s = AppSettings.fromServer(
            listOf(ns("agent-presets", JSONObject().put("default", JSONObject.NULL))),
        )
        assertEquals("standard", s.agentPreset)
    }
    ...
    fun `读回值与提交值不一致时视为保存失败`() {
        ...
        assertTrue(ex.message!!.contains(dev.deeplinks.core.DshStringsZh.saveVerifyFailed.format("agent-presets")))
    }
