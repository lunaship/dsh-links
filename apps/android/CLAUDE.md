...
DSH Links Android 客户端（私有仓）。配对插件在 `../dsh-links`；版本基线以 `../dsh-links/docs/COMPATIBILITY.md` 为准，本仓库不另维护版本表。
...
- 正式包只发签名 APK（签名环境自动读取 `~/Library/Application Support/DSH Links Signing/env`）；**不要**把本仓库推到公开的 `lunaship/dsh-links`。
